...
- Claude 真正在工作时（处理 prompt、调用工具），自动弹出一个 DOS 游戏风格的大画面动画窗口，跟着两个角色（**大彩色章鱼** + **随机 buddy 教练**）做 7 组循环动作。
- 顶部大字 banner 显示英文动作名（FIGlet block 字体），中央"练习画布"用大字符元素（大箭头、巨型方框、大手掌）画动画，右下角两个角色实时切换姿势 + 说台词。
...
| 类别 | 动作 | 时长 | 怎么做 |
|---|---|---|---|
| 眼动 | **眼神追踪** | 60s | 大彩色章鱼的眼睛切换 5 个方向（中/上/下/左/右），中央画一个超大方向箭头，你的眼睛跟着大章鱼视线方向看 |
...
1. **大彩色章鱼**（28×22 像素，24-bit RGB 粉色 + dithering 立体感）——固定出场。在 `eye-track` 动作里它的眼睛会切换方向，你跟着它看。其他动作时它在做 idle 表情（偶尔眨眼、左右瞄）。
2. **随机 buddy 教练**（5×12 字符，移植自 Claude Code 内部的 buddy sprite）——每次会话从 5 个里随机一个（基于 session_id 哈希，所以同一会话稳定）：
   - 🐱 **cat** 猫教练
   - 🦫 **capybara** 水豚教练
   - 🤖 **robot** 机器人教练
   - 🐙 **octopus** 小章鱼教练（和大章鱼组成"章鱼组合"）
   - 🐲 **dragon** 龙教练

   buddy 每秒切一帧（3 帧待机动画），并有气泡台词随动作切换。

## 大字 banner

顶部 5 行 banner 用手写 FIGlet block 字体显示当前动作的英文名（EYE TRACK / SACCADE / FINGERS / HAND POSE / SHOULDER / NECK SIDE / BREATHE），不依赖外部 figlet 命令。
...
│   ├── open.sh                    幂等开启窗口
│   ├── close.sh                   同步关闭并 wait
│   ├── animation.sh               主动画进程 + 7 个 canvas 渲染器
│   ├── buddy.sh                   5 种 buddy sprite 表（移植自 Claude Code）
│   ├── banner.sh                  A-Z + 0-9 block 字体
│   ├── octopus_big.sh             大彩色章鱼（28×22 像素，6 种眼神 pose）
│   ├── render-stats.sh            统计面板渲染
│   └── lib.sh                     公共：原子锁、liveness、配置读取
└── skills/stats/SKILL.md          /claude-coach:stats 触发器

动画完全程序化，无外部帧文件——每个动作一个 render_* 函数，按真实节奏（0.2s 帧间隔）画下一帧。所有 sprite 数据内联在 bash 字符串里，纯 ASCII + 半块字符 ▀ + ANSI 24-bit RGB。
```
